...
The implementation, the frozen pilot bundle, the fixture and assignment manifests, the generated result fragments, and the build are available to reviewers as an anonymized archive; a DOI-bearing deposit will replace it at camera-ready. The bundle carries a SHA-256 manifest covering 35 artifacts and 20 declared inputs; all 20 inputs resolve at commit \texttt{26724a6}. It binds 64 provenance rows (43 executed fixture rows and 21 aggregate rows). The packet records \texttt{git.dirty=true}, so a clean-commit, tagged recapture remains pending before archival release.
