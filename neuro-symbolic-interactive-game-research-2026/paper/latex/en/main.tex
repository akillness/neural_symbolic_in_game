...
The repository contains the implementation, pilot bundle, fixture and assignment manifests, generated result fragments, and build sources. The current Stage-10 SHA-256 manifest covers 35 artifacts and 21 declared inputs and binds 64 provenance rows (43 executed fixture rows and 21 aggregate rows). It records \texttt{dirty=false} at input commit \texttt{c4752df}, tagged \texttt{trace-rpg-stage10-inputs-20260814-v1}. All 35 artifact hashes and all 21 input hashes recompute, and every input path and exact digest matches that commit (56/56 total hashes). The manifest records the portable invocation \texttt{uv run python} and contains no absolute user or clone path. This closes the clean-input reproducibility gate; it does not mean that an anonymized reviewer archive or DOI-bearing deposit already exists, and neither is claimed here.
