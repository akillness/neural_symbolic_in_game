...
The implementation, the frozen pilot bundle, the fixture and assignment manifests, the generated result fragments, and the build are available to reviewers as an anonymized archive; a DOI-bearing deposit will replace it at camera-ready. The bundle carries a SHA-256 manifest covering 32 artifacts and 14 declared inputs. Reviewers should note that the manifest's recorded provenance commit predates the pilot runner and does not resolve every declared input; the packet is being regenerated from a clean committed tree so that every input hash-matches at the recorded revision.
