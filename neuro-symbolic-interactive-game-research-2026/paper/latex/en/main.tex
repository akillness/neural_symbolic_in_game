...
\begin{enumerate}
  \item \emph{versioned trust-boundary contracts} for state, policy, candidate, record, and game bridge, with a type-strict parser that rejects ambiguous known-field types and unknown candidate keys at proposal and replay boundaries;
  \item \emph{a validate--repair--commit controller} with six state-relative predicates, bounded repair, unchanged fallback, and defensive pre-commit revalidation;
  \item \emph{an audit-evidence layer} using unkeyed SHA-256 content links, state-semantic replay, and episode-continuity checks;
  \item \emph{an offline conformance harness} with frozen assignment keys, classified terminal failures, and assignment-complete accounting, including failures for which no proposal trace can exist; and
  \item \emph{a counterexample-guided operator} $\rho(a,E)$ that reads only the prior candidate and structured errors, compared with blind unchanged retry and a state-reading oracle upper bound across frozen repairability classes.
\end{enumerate}
Deterministic parser, state-isolation, repair, fault-injection, and manifest fixtures provide the exact-count evidence for these five implementation contributions.
