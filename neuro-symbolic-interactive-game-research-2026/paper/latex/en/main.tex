...
Large language models were used as authoring and review tools in preparing this manuscript: to draft and revise prose, to simulate a reviewer panel whose findings drove this revision, and to run citation-identity checks against bibliographic metadata APIs. They were not used to generate, alter, or interpret any experimental result. All reported numbers are emitted by the deterministic pilot runner from frozen fixtures and are inserted into the manuscript only through machine-generated fragments. Every claim about the implementation was verified by the authors against the source, and every citation was verified against at least one independent index. The authors take full responsibility for the content.

\section*{Data and Code Availability}
The implementation, the frozen pilot bundle, the fixture and assignment manifests, the generated result fragments, and the build are available to reviewers as an anonymized archive; a DOI-bearing deposit will replace it at camera-ready. The bundle carries a SHA-256 manifest covering 35 artifacts and 20 declared inputs; all 20 inputs resolve at commit \texttt{26724a6}. Its 64 manifest keys comprise 43 executed-fixture rows and 21 aggregate rows. The packet records \texttt{git.dirty=true}, so a clean-commit, tagged recapture remains pending before archival release.
