...
구현, 동결 파일럿 번들, fixture 및 assignment manifest, 생성 결과 조각, 빌드는 심사용 익명 아카이브로 제공하며 camera-ready 시 DOI 기반 저장소로 대체한다. 번들의 SHA-256 manifest는 산출물 35개와 선언 입력 20개를 포함하며, 입력 20/20은 commit \texttt{26724a6}에서 모두 해소된다. Manifest key 64개는 실행 fixture row 43개와 aggregate row 21개로 구성된다. 다만 패킷은 \texttt{git.dirty=true}를 기록하므로 archival release 전에 clean commit과 tag에서 재캡처해야 한다.
