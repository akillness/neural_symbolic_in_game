...
Repository에는 구현, 파일럿 번들, fixture 및 assignment manifest, 생성 결과 조각, build source가 포함된다. 현재 Stage-10 SHA-256 manifest는 산출물 35개와 선언 입력 21개를 포함하고 bound provenance row 64개(실행 fixture row 43개와 aggregate row 21개)를 결합한다. 입력 commit \texttt{c4752df}에는 \texttt{trace-rpg-stage10-inputs-20260814-v1} tag가 있으며 manifest는 \texttt{dirty=false}를 기록한다. 산출물 hash 35개와 입력 hash 21개가 모두 재계산되고, 모든 입력 경로와 정확한 digest가 해당 commit과 일치한다(총 56/56). Manifest의 실행 명령은 이식 가능한 \texttt{uv run python}이며 절대 사용자 경로나 clone 경로를 포함하지 않는다. 이로써 clean-input 재현성 gate는 종결되지만, 익명 심사 archive 또는 DOI 기반 저장소가 이미 존재한다는 뜻은 아니며 본 원고는 어느 것도 주장하지 않는다.
