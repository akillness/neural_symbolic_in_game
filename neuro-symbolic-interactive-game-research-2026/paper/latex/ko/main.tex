...
구현, 동결 파일럿 번들, fixture 및 assignment manifest, 생성 결과 조각, 빌드는 심사용 익명 아카이브로 제공하며 camera-ready 시 DOI 기반 저장소로 대체한다. 번들은 산출물 35개, 선언 입력 20개, assignment key 64개를 포함하는 SHA-256 manifest를 갖는다. 다만 manifest에 기록된 provenance commit은 파일럿 runner보다 앞서며 선언된 모든 입력을 해석하지 못한다. 모든 입력이 기록된 revision에서 해시 일치하도록 패킷을 깨끗한 커밋 트리에서 재생성 중이다.
