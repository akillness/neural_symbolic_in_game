...
class ConformancePilotTests(unittest.TestCase):
    ...
    def test_generated_artifacts_schemas_hashes_and_release_mirror(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIsInstance(reproducibility["git"]["tags_at_commit"], list)
            ...
            self.assertEqual(
                reproducibility["environment"]["python_invocation"], ["uv", "run", "python"]
            )
            self.assertNotIn("python_executable", reproducibility["environment"])
            self.assertTrue(reproducibility["environment"]["platform"])
            self.assertTrue(reproducibility["environment"]["uv_version"])
            serialized_reproducibility = json.dumps(reproducibility, sort_keys=True)
            self.assertNotIn(str(Path.home()), serialized_reproducibility)
            self.assertNotIn(str(ROOT), serialized_reproducibility)

            with self.assertRaisesRegex(RuntimeError, "clean Git input tree"):
                pilot._validate_release_git_binding(
                    {"dirty": True, "tags_at_commit": ["input-v1"]}, True
                )
            with self.assertRaisesRegex(RuntimeError, "at least one Git tag"):
                pilot._validate_release_git_binding({"dirty": False, "tags_at_commit": []}, True)
