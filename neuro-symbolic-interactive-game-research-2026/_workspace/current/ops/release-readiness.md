...
| Curated tracked Higgsfield player | DONE-PRODUCTION | validator PASS: 9,677,324-byte GLB, 15,463 triangles, 24 joints, `Idle`/`Casual_Walk`; presentation-only; deployed in `dpl_GpRiuFSFGPrbbVMmFsPdPq731f9Y` |
...
| Staged Web artifact | DONE-PRODUCTION | current 11 manifest files / 50,745,187 bytes; 10 runtime files / 50,744,674 bytes; PCK 10,892,412 bytes, SHA-256 `29e3d8b6…8f76`; includes curated player; deployed production is byte-identical |
| Current gameplay recording | DONE-WORKING-CAPTURE | `trace-rpg-gameplay.mp4`: Compresso H.264 High, 1280×720, 30 fps, 69.067 s, 5,662,128 bytes, SHA-256 `aa374c5a…7044`; full authored route; engineering demonstration only |
| Production HTTP/MIME | DONE | HTML/JS/WASM/PCK/OFL returned 200; WASM `application/wasm`; OFL `text/plain` |
| Deployed browser layout/glyph smoke | DONE-BOUNDED | current 2026-08-30 desktop start/in-game/Field Guide smoke, zero console/page errors; retained 2026-08-17 1280×720 and 390×844 captures are historical; current mobile recheck open |
...
| Deployment/response verification | DONE-BOUNDED | production `dpl_GpRiuFSFGPrbbVMmFsPdPq731f9Y` READY at `https://sealed-lighthouse-trace-rpg.vercel.app`; 10 runtime files 200 and byte-identical; `vercel.json` correctly non-public |
...
`dpl_GpRiuFSFGPrbbVMmFsPdPq731f9Y`; bounded production desktop smoke and exact runtime-byte checks
passed. Pending-review concept candidates remain excluded. The extension tab's `WrongDocumentError`
