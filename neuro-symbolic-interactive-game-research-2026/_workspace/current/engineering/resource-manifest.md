...
| `game-track/web/vercel.dashboard.json` + `scripts/deploy_vercel_dashboard.sh` | Web release lane (dashboard site) | root redirect to `/dashboard/`, game at `/public/`, `outputDirectory: dist`, same static security headers | `[OBSERVED]` production deployment `dpl_AMVHgXcYKRgqz9UCyNUBfLhsfB3Y` (2026-09-03), receipts in `qa/browser-qa.md` |
