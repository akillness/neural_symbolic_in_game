...
| DEF-009 | S2 | Web export | playable scene previously had no reproducible Web preset/staged release path | resolved: 11-file artifact deployed; latest PCK 1,573,072 bytes after `docs/latest/**` exclusion; HTML/JS/WASM/PCK/OFL 200, correct WASM/OFL MIME | release engineer |
| DEF-010 | S2 | browser input/audio | pointer capture and autoplay require a real user gesture and recovery path | resolved for bounded smoke: trusted headless Playwriter click entered pointer lock with zero console/page errors; input latency still DEF-014 | UI/audio lane + QA |
...
| DEF-017 | S3 | Playwriter extension inspection | extension-connected tab raised `WrongDocumentError` because it did not own the root document for pointer lock | closed-automation-only; dedicated trusted headless sessions passed with zero page/console errors | QA automation |
