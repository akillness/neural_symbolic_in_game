...
- Follow-up redeploy `dpl_AMVHgXcYKRgqz9UCyNUBfLhsfB3Y` (supersedes `dpl_G58qo7siKGUP2E55RH2Xxj8pPgtF`, same staged bytes except `dashboard.js`): the dashboard's `message` handler now accepts envelopes only from the embedded game frame on the same origin (`event.source === #game.contentWindow && event.origin === location.origin`). Production check: real frame envelopes (`tick`, then the initial snapshot after BEGIN) still update the header to `state f488d9c4…812c`; a replayed real envelope with every 64-hex hash replaced by `deadbeef…`, posted from the top window, changed nothing and no spoofed hash appeared on the page. `/public` → 308 → `/public/` (200 text/html), so the README "game only" badge target resolves. Zero 4xx, zero console/page errors.
