...
| EIC-9 | Conditional-compilation scaffolding must not reach submission | `PARTIALLY_ADDRESSED` |
...
**EIC-9 — partial.** The scaffolding is still present. It is latent — the current build contains no
placeholder text, verified — but the reviewer is right that a submission bundle should not carry
fallback machinery for its own results. Recorded as F17 for camera-ready.
...
| R3-4 | Three of five floats are never cited in either language | `PARTIALLY_ADDRESSED` |
...
**R3-4 — partial.** Not yet fixed; recorded as F18 for camera-ready.
...
| DA-11 | Submitted with a conditional build harness in place of finished figures | `PARTIALLY_ADDRESSED` |
...
Closed by ID: I1, I2, I3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F14, F15, F16. F13 and F14 were the two acceptance blockers carried out of Stage 8's first pass and are now closed: the propositions are relabelled as asserted invariants with the vacuous antecedent disclosed, and every manifest row carries a schema-enforced executed-or-aggregate class with the split declared and asserted.

Open by ID: F17 and F18, both camera-ready copy-edit items.

Partially addressed comments: DA-3, DA-11, EIC-9, EIC-10, R1-1, R1-5, R3-4. Rebutted: DA-8, DA-10, EIC-11, R2-5. Each is argued in place above.

We state closure by finding ID rather than by class, because the Stage 6 classes and the reviewer comment set do not partition the same way: a comment can remain partial while the finding it maps to is closed. No acceptance blocker remains open; two copy-edit items do, and we do not represent the manuscript as camera-ready.

Class A 무결성 지적과 Class B 범위 지적은 모두 종결됐다. 게재 승인 차단 항목 두 건(F13, F14)과
camera-ready 항목 두 건(F17, F18)은 미해결 상태로 남아 있으며 material passport에 그대로
기록했다. 본 개정을 완결로 표현하지 않는다.
