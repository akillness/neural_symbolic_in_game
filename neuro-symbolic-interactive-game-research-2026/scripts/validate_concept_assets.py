...
PUBLIC_EXCLUSION_PATH = ASSET_ROOT / "public-exclusion.json"
...
def validate_public_exclusion() -> int:
    exclusion = json.loads(PUBLIC_EXCLUSION_PATH.read_text(encoding="utf-8"))
    if exclusion.get("profile") != "public-safe-latest-snapshot":
        raise ValueError("unexpected public exclusion profile")
    if exclusion.get("primary_experiment_eligible") is not False:
        raise ValueError("public exclusion entered the primary experiment")
    if exclusion.get("runtime_eligible") is not False:
        raise ValueError("public exclusion entered the runtime")
    rows = exclusion.get("omitted_assets", [])
    ids = {row.get("asset_id") for row in rows}
    if ids != PUBLIC_OMITTED_IDS or len(rows) != len(PUBLIC_OMITTED_IDS):
        raise ValueError(f"unexpected public exclusion set: {sorted(ids)}")
    for row in rows:
        relative = Path(row["file"])
        if relative.is_absolute() or ".." in relative.parts or relative.suffix.lower() != ".png":
            raise ValueError(f"unsafe excluded asset path: {relative}")
        if not re.fullmatch(r"[0-9a-f]{64}", row.get("sha256", "")):
            raise ValueError(f"invalid excluded asset digest: {row['asset_id']}")
        if (ASSET_ROOT / relative).exists():
            raise ValueError(f"publication-blocked byte present: {relative}")
    unexpected_pngs = sorted(path.relative_to(ASSET_ROOT) for path in ASSET_ROOT.rglob("*.png"))
    if unexpected_pngs:
        raise ValueError(f"unexpected public PNG bytes: {unexpected_pngs}")
    print(f"PASS public-safe exclusion manifest: {len(rows)} image byte streams absent")
    return 0


def main() -> int:
    if PUBLIC_EXCLUSION_PATH.exists():
        compatibility = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
        if compatibility.get("profile") != "public-safe-latest-snapshot":
            raise ValueError("public exclusion requires the public-safe compatibility manifest")
        if {row.get("asset_id") for row in compatibility.get("assets", [])} != {
            "SL-C01",
            "SL-C02",
            "SL-C03",
            "SL-C04",
        }:
            raise ValueError("public-safe compatibility asset IDs drifted")
        return validate_public_exclusion()
    if not MANIFEST_PATH.exists():
        raise ValueError("neither internal asset manifest nor public exclusion exists")
