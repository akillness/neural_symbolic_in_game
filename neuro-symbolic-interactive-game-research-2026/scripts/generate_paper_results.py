...
별도 스크리닝 파일럿은 hosted API를 통해 {model_id}을 호출했다. 셀마다 seed로 구분한 제안 호출 {call_count}회와 수리 예산 $K={repair_budget}$을 사용했고, 각 호출의 최초 후보 하나를 블라인드 동일 후보 retry와 $\rho$가 공유했다. Hosted revision은 고정되지 않았고 token 회계는 제공되지 않았으며 seed는 hosted sampler를 제어하지 않으므로 이 호출들은 독립 무작위 표본이 아니라 준반복이다. 따라서 추론 통계나 모델 순위를 보고하지 않는다.
